...
import surrogate
from baxus_embedding.utils import from_unit_cube


def pick_best_from_configurations(
        initializers: List[Callable[["surrogate.gp.GP"], None]],
        model: "surrogate.gp.GP",
        train_x: torch.Tensor,
        train_y: torch.Tensor,
        n_best: Optional[int] = 1,
) -> List[Callable[["surrogate.gp.GP"], None]]:
    """
...
def mle_optimization(
        initializer: Callable[["surrogate.gp.GP"], None],
        model: "surrogate.gp.GP",
        num_steps: int,
        train_x: torch.Tensor,
        train_y: torch.Tensor,
...
def latin_hypercube_hp_grid(
    ...
    for k, v in hp_grid.items():
        ...
        keys.append(k)
    d = len(keys)
    sampler = qmc.LatinHypercube(d=d, seed=np.random.randint(1e6))

...
# Author: Leonard Papenmeier <leonard.papenmeier@cs.lth.se> (https://github.com/LeoIV/BAxUS/blob/main/baxus/gp.py)
...
from surrogate.gp_configuration import MLLEstimation, GPBehaviour
from surrogate.gp_utils import (
    initializer_factory,
    mle_optimization,
...
class GP(SingleTaskGP):
    ...
    def __init__(
        ...
        super(GP, self).__init__(
            train_X=train_x,
            train_Y=torch.unsqueeze(train_y, 1) if train_y.ndim == 1 else train_y,
...
def train_gp(
    ...
    ard_dims = (
        (train_x.shape[1]) if gp_behaviour.use_ard else None
    )
    model = GP(
        train_x=train_x,
        train_y=train_y,
